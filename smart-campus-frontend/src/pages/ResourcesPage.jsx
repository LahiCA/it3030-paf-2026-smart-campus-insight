  ...
  return (
    <div>
      ...
      <div className="toolbar">
        <div className="search-box">
          <Search size={14}/>
          <input placeholder="Search by name, location or description…" value={search} onChange={e => setSearch(e.target.value)}/>
        </div>
        <select className="filter-select" value={typeFilter} onChange={e => setTypeFilter(e.target.value)}>
          <option value="">All Types</option>
          {TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g,' ')}</option>)}
        </select>
        <input className="filter-input" type="text" placeholder="Location" value={locationFilter} onChange={e => setLocationFilter(e.target.value)} />
        <input className="filter-input" type="number" min={1} placeholder="Min capacity" value={capacityMin} onChange={e => setCapacityMin(e.target.value)} />
      ...
      {loading
        ? <div className="loading-container"><div className="spinner"/></div>
        : <ResourceCard resources={filtered} onEdit={r => { setEditing(r); setShowModal(true) }} onDelete={handleDelete} canManage={canManage}/>
      }
