...
const ResourcesPageTailwind = () => {
  ...
  const handleImageError = (resourceId) => {
    setFailedImages((prev) => new Set([...prev, resourceId]));
  };
  ...
  if (loading) {
    return (
      <div className="flex min-h-64 items-center justify-center">
        <div className="h-12 w-12 animate-spin rounded-full border-[3px] border-slate-200 border-t-teal-600" />
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
          {filteredResources.map((resource) => (
            ...
            >
              ...
              <div className="relative flex h-28 items-center justify-center overflow-hidden bg-[linear-gradient(135deg,#F0FDFA_0%,#CCFBF1_50%,#99F6E4_100%)]">
                {resource.imageUrl && !failedImages.has(resource.id) ? (
                  <img
                    src={`${BACKEND_URL}${resource.imageUrl}`}
                    alt={resource.name}
          ...
          <div className="max-h-[90vh] w-full max-w-130 overflow-y-auto rounded-2xl bg-white shadow-2xl scrollbar-ui">
            ...
            <form onSubmit={handleSubmit} className="px-6 pb-6">
              <div className="mb-4 grid gap-4 md:grid-cols-2">
                ...
                {/* Resource Name — plain text input */}
                <div>
                  <label className="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-700">
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  className={`${inputClassName} max-w-50`}
                >
                  {STATUS_OPTIONS.map((s) => (
                    <option key={s} value={s}>{s}</option>
                  ))}
...
// const BACKEND_URL = 'http://localhost:8080'; // adjust to your backend origin
...
//       {filteredResources.length === 0 ? (
//         <div className="rounded-3xl border border-slate-100 bg-white px-6 py-16 text-center shadow-sm">
//          <div className="flex h-28 items-center justify-center overflow-hidden bg-[linear-gradient(135deg,#F0FDFA_0%,#CCFBF1_50%,#99F6E4_100%)]">
//             {resource.imageUrl ? (
//               <img
//                 src={`${BACKEND_URL}${resource.imageUrl}`}
//                 alt={resource.name}
