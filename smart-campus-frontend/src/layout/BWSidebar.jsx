...
function BWSidebar() {
  ...
  const menuItems = [
    ...
    { path: '/bw-create-booking', icon: FaCalendarPlus, label: 'Create Booking' },
    { path: '/bw-my-bookings', icon: FaClipboardList, label: 'My Bookings' }
  ];

  if (isAdmin()) {
    menuItems.push({ path: '/bw-admin-bookings', icon: FaUsersCog, label: 'Admin Bookings' });
  }
    ...
    <aside className="sticky top-0 flex h-screen w-[72px] min-w-[72px] flex-col border-r border-slate-100 bg-white font-['Poppins',sans-serif] md:w-[260px] md:min-w-[260px]">
      <div className="flex justify-center gap-3 border-b border-slate-100 px-3 py-5 md:justify-start md:px-5 md:pb-5 md:pt-6">
        ...
        <div className="hidden flex-col md:flex">
          ...
          <span className="text-[11px] font-medium uppercase tracking-[1px] text-slate-400">Booking</span>
        </div>
      </div>
